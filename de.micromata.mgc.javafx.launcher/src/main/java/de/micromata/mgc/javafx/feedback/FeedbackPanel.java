...
public class FeedbackPanel extends GridPane implements Initializable
{
   ...
   */
  public final void addMessage(ValMessage message)
  {
   ...
   */
  public final void addMessages(Collection<ValMessage> messages)
  {
    for (ValMessage li : messages) {
      messageToRow(li);
    }
   ...
   */
  private Parent messageContainer(final ValMessage message, final RowConstraints row)
  {
    ...
    final Circle dot = CircleBuilder.create().radius(LI_RADIUS).strokeWidth(0).styleClass(message.getValState().name())
        .id("icon").build();
